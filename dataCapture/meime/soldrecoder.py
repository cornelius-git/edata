...
class salerecoder:
    ...
    def sqlreader(self,start,end):
        ...
        for i in range(start,end):
            self.itemid = None
            try:
                sql = "select itemid from airsuspension where id={}".format(i)
                cur.execute(sql)
                self.itemid = cur.fetchall()[0][0]
                #已存在的库中判断
                print(self.itemid)
                sql1 = "SELECT * FROM airsuspensionsold WHERE itemid = {}".format(self.itemid)
                cur.execute(sql1)
                f = cur.fetchall()
                print(cur.fetchall())
                if len(f)>0:
    ...
    def datawriter(self,price=None,quantity=None,deal_time=None,source=None):
        sql2 = "insert into airsuspensionsold(itemid,price,Quantity,Date_of_Purchase,source) values(%s,%s,%s,%s,%s)"
        cur.execute(sql2,(self.itemid,price,quantity,deal_time,source))
        db.commit()
    def recoder(self):
        headers = {
            'User-Agent': user_agent[random.randint(0, 35)]}
        rty = open("airsuspension1.text",mode='a+',encoding='utf-8')
...
if __name__ == '__main__':
    salerecoder().sqlreader(start=195364,end=224352)